...
#         f.close()
    ...
    def __getDataMatrix(self, scope):
        ...
        if(self.__isFirstTime):
            ...
            docInfo = map(self.__doc2Matrix, self.labels.items())
            if(len(docInfo) == 0):
                print "Lost doc: ", self.labels.items()
                return None
        ...
        docInfo = self.__docInfo
        docMatrixes, docSentenceNums, sentenceWordNums, labels = zip(*docInfo)
            
        return (docMatrixes[scope[0]:scope[1]], \
                docSentenceNums[scope[0]:scope[1]], 
                sentenceWordNums[scope[0]:scope[1]], 
                labels[scope[0]:scope[1]])
        
    def __getDataMatrix2(self, scope):
        scope[1] = np.min([scope[1], len(self.labels)])
        if(scope[0] < 0 or scope[0] >= scope[1]):
            return None
